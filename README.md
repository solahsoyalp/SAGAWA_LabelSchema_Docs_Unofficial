...
`samples` フォルダに、各レイアウト（取込／出力）の **列名のみ（ヘッダ1行）** のサンプルCSVを用意しています。CSV作成時の項目順・項目名の雛形としてご利用ください。

- 対象は CSV 項目表を持つレイアウト12ファイルです（コード一覧・離島地域は除く）。
- ファイルは **UTF-8** で保存しています。e飛伝Ⅲへ取り込む際は **Shift_JIS（CP932）** への変換が必要な場合があります。
...
3. `samples` フォルダの各CSVを雛形として利用できます。
